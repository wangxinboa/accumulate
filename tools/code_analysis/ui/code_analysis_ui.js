...
links.forEach((link) => {
	const line = new fabric.Polyline([
		...
		{ x: startX + link.source.depth * nodeSize + nodeSize + 2.5, y: startY + link.target.index * nodeSize + 8 },
	], {
		strokeWidth: 1,
...
nodes.forEach((node) => {
	const font = new fabric.Text(node.data.name, {
		top: startY + node.index * nodeSize,
		left: startX + node.depth * nodeSize + 8,
		fontSize: 14,
		height: 20,
	});
	canvas.add(font);
	...
	const circle = new fabric.Circle({
		originX: 'center',
		originY: 'center',
		top: startY + node.index * nodeSize + 8,
		left: startX + node.depth * nodeSize + 2,
		radius: 2.5,
