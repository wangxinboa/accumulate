import { GLTFLoaderState, GLTFLoaderCoordinateSystemMode, GLTFLoaderAnimationStartMode } from '../glTFFileLoader.js';
import {
	...
	TextureMagFilter,
	MeshPrimitiveMode,
} from "../../../public/babylon.glTF2Interface.js";

import { nodeAnimationData } from "./glTFLoaderAnimation.js";

const {
	Deferred,
	Quaternion, Vector3, Matrix, TmpVectors,
	Color3,
	Tools,
	Camera,
	FreeCamera,
	...
	MorphTarget,
	MorphTargetManager,
	DecodeBase64UrlToBinary, IsBase64DataUrl, LoadFileError,
	Logger,
	BoundingInfo,
} = window.BABYLON;
...
export class GLTFLoader {
	...
	/** @internal */
	_allMaterialsDirtyRequired = false;
	...
	_bin = null;
	_babylonScene;
	_rootBabylonMesh = null;
	_defaultBabylonMaterialData = {};
	_postSceneLoadActions = [];

	static _RegisteredExtensions = {};

	/**
	 * The default glTF sampler.
	 */
	static DefaultSampler = { index: -1 };
	...
	_loadBone(node, skin, babylonSkeleton, babylonBones) {
		...
		if (node.index !== skin.skeleton) {
			if (node.parent && node.parent.index !== -1) {
				...
				Logger.Warn(`/skins/${skin.index}/skeleton node is not a common root`);
			}
		}
	...
	_loadAccessorAsync(context, accessor, constructorypedArrayConstructor) {
		if (accessor._data) {
	...
	_createDefaultMaterial(name, babylonDrawMode) {
		...
		this._babylonScene._blockEntityCollection = false;
		// Moved to mesh so user can change materials on gltf meshes: babylonMaterial.sideOrientation = this._babylonScene.useRightHandedSystem ? Material.CounterClockWiseSideOrientation .ClockWiseSideOrientation;
	...
	_extensionsLoadMeshPrimitiveAsync(
		context,
		name,
		node,
		mesh,
