...
import { AnimatedSprite } from '../engine/animatedSprite.ts';
import Resources from '../engine/resources.ts';
import SpriteCuts from './spriteCuts.ts';
import Mario from './setup.ts';
import ImprovedNoise from './improvedNoise.ts';
import { LevelType, LevelTypeType } from './level.ts';
import { KeyboardInput, Keys } from '../engine/keyboardInput.ts';
import WinState from './winState.ts';
...
export const MapTile: MapTileType = {
	Grass: 0,
	Water: 1,
	Level: 2,
	Road: 3,
...
export default class MapState extends GameState {
	camera: Camera;
	...
	WaterSprite?: AnimatedSprite;
	DecoSprite?: AnimatedSprite;
	HelpSprite?: AnimatedSprite;
	SmallMario?: AnimatedSprite;
	LargeMario?: AnimatedSprite;

	FontShadow?: SpriteFont;
	Font?: SpriteFont;
	...
	constructor() {
		...
		this.XFarthestCap = 0;
		this.YFarthestCap = 0;
		this.MapImage = (document.createElement("canvas") as HTMLCanvasElement);
		this.MapImage.width = 320;
		this.MapImage.height = 240;
		this.MapContext = (this.MapImage.getContext("2d") as CanvasRenderingContext2D);
	...
	Connect(xSource: number, ySource: number, width: number, height: number): void {
		var maxDistance = 10000, xTarget = 0, yTarget = 0, x = 0, y = 0, xd = 0, yd = 0, d = 0;
	...
	RenderStatic(): void {
		var x = 0, y = 0, p0 = 0, p1 = 0, p2 = 0, p3 = 0, s = 0, xx = 0, yy = 0, image = Resources.Images["worldMap"], type = 0;
