...
import TitleState from './titleState.ts';
...
export default class LoadingState extends GameState {
	...
	Update(delta: number): void {
		...
		(this.ScreenColor as number) += this.ColorDirection * 255 * delta;
		if ((this.ScreenColor as number) > 255) {
			this.ScreenColor = 255;
			this.ColorDirection = -1;
		} else if ((this.ScreenColor as number) < 0) {
			this.ScreenColor = 0;
			this.ColorDirection = 1;
		}
	}
	Draw(context: CanvasRenderingContext2D): void {
		if (!this.ImagesLoaded) {
			var color = parseInt((this.ScreenColor as string), 10);
