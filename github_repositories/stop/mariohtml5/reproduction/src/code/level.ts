...
export type LevelTypeType = LevelType.Overground | LevelType.Underground | LevelType.Castle;
...
export class Level {
	...
	SpriteTemplates: Array<Array<SpriteTemplate | null>>;
	...
	GetSpriteTemplate(x: number, y: number): SpriteTemplate | null {
		if (x < 0) { return null; }
		if (y < 0) { return null; }
	...
	SetSpriteTemplate(x: number, y: number, template: SpriteTemplate): void {
		if (x < 0) { return; }
		if (y < 0) { return; }
		if (x >= this.Width) { return; }
		if (y >= this.Height) { return; }
