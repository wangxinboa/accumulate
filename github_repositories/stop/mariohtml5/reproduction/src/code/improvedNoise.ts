...
export default class ImprovedNoise {
	...
	Noise(x: number, y: number, z: number): number {
		...
		var A = this.P[nx] + ny, AA = this.P[A] + nz, AB = this.P[A + 1] + nz, B = this.P[nx + 1] + ny, BA = this.P[B] + nz, BB = this.P[B + 1] + nz;
	...
	Lerp(t: number, x: number, y: number): number {
		return x + t * (y - x);
	}
