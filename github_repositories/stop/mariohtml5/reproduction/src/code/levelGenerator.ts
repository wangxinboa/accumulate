import { Odds, Level, LevelType, LevelTypeType } from './level.ts';
...
export default class LevelGenerator {
	...
	CreateLevel(type: LevelTypeType, difficulty: number): Level {
		var i = 0, length = 0, floor = 0, x = 0, y = 0, ceiling = 0, run = 0, level: null | Level = null;
	...
	BuildZone(level: Level, x: number, maxLength: number): number {
	...
	BuildJump(level: Level, xo: number, maxLength: number): number {
	...
	BuildCannons(level: Level, xo: number, maxLength: number): number {
		alert("cannons");
		var length = ((Math.random() * 10) | 0) + 2, floor = this.Height - 1 - (Math.random() * 4) | 0, xCannon = xo + 1 + (Math.random() * 4) | 0, x = 0, y = 0, cannonHeight = 0;
	...
	BuildHillStraight(level: Level, xo: number, maxLength: number): number {
		var length = ((Math.random() * 10) | 0) + 10, floor = this.Height - 1 - (Math.random() * 4) | 0, x = 0, y = 0, h = floor, keepGoing = true, l = 0, xxo = 0, occupied: Array<boolean> = [], xx = 0, yy = 0;
	...
	AddEnemyLine(level: Level, x0: number, x1: number, y: number): void {
		var x = 0, type = 0;
		for (x = x0; x < x1; x++) {
	...
	BuildTubes(level: Level, xo: number, maxLength: number): number {
	...
	BuildStraight(level: Level, xo: number, maxLength: number, safe: boolean): number {
	...
	Decorate(level: Level, x0: number, x1: number, floor: number): void {
		if (floor < 1) {
			return;
		}
	...
	FixWalls(level: Level): void {
		var blockMap: Array<Array<boolean>> = [], x = 0, y = 0, xx = 0, yy = 0, blocks = 0;
	...
	Blockify(level: Level, blocks: Array<Array<boolean>>, width: number, height: number): void {
		var to = 0, b: Array<Array<boolean>> = [], x = 0, y = 0, xx = 0, yy = 0, i = 0, _xx = 0, _yy = 0;
