 ...
 */
const createPolyPositionHandler = codeMarkFunction(pointIndex => {
 ...
 */
const polyActionHandler = codeMarkFunction((eventData, transform, x, y) => {
	const {
		target,
		pointIndex
	} = transform;
 ...
 */
const factoryPolyActionHandler = codeMarkFunction((pointIndex, fn) => {
...
const createPolyActionHandler = codeMarkFunction(pointIndex => wrapWithFireEvent(ACTION_NAME, factoryPolyActionHandler(pointIndex, polyActionHandler)), 'createPolyActionHandler');
const createPolyControls = codeMarkFunction(function createPolyControls(arg0) {
