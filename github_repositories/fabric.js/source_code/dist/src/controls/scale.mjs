 ...
 */
const scaleIsProportional = codeMarkFunction(function scaleIsProportional(eventData, fabricObject) {
 ...
 */
const scalingIsForbidden = codeMarkFunction(function scalingIsForbidden(fabricObject, by, scaleProportionally) {
 ...
 */
const scaleCursorStyleHandler = codeMarkFunction((eventData, control, fabricObject) => {
 ...
 * @private
 */
const scaleObject = codeMarkFunction(function scaleObject(eventData, transform, x, y) {
 ...
 * @return {Boolean} true if some change happened
 */
const scaleObjectFromCorner = codeMarkFunction((eventData, transform, x, y) => {
 ...
 * @return {Boolean} true if some change happened
 */
const scaleObjectX = codeMarkFunction((eventData, transform, x, y) => {
	return scaleObject(eventData, transform, x, y, {
		by: 'x'
	});
}, 'scaleObjectX');
 ...
 * @return {Boolean} true if some change happened
 */
const scaleObjectY = codeMarkFunction((eventData, transform, x, y) => {
	return scaleObject(eventData, transform, x, y, {
		by: 'y'
	});
}, 'scaleObjectY');
const scalingEqually = wrapWithFireEvent(SCALING, wrapWithFixedAnchor(scaleObjectFromCorner));
const scalingX = codeMarkFunction(wrapWithFireEvent(SCALING, wrapWithFixedAnchor(scaleObjectX)), 'scalingX');
const scalingY = codeMarkFunction(wrapWithFireEvent(SCALING, wrapWithFixedAnchor(scaleObjectY)), 'scalingY');
