...
const isCollection = codeMarkFunction(fabricObject => {
	return !!fabricObject && Array.isArray(fabricObject._objects);
}, 'isCollection');
const createCollectionMixin = codeMarkFunction(function createCollectionMixin(Base) {
	const Collection = codeMarkClass(class Collection extends Base {
		constructor() {
			super(...arguments);
			/**
			 * @type {FabricObject[]}
