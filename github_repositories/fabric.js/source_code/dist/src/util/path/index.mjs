 ...
 */
const segmentToBezier = codeMarkFunction((theta1, theta2, cosTh, sinTh, rx, ry, cx1, cy1, mT, fromX, fromY) => {
 ...
 * @param rotateX
 */
const arcToSegments = codeMarkFunction((toX, toY, rx, ry, large, sweep, rotateX) => {
	if (rx === 0 || ry === 0) {
		return [];
	}
	let fromX = 0,
 ...
 */
const calcVectorAngle = codeMarkFunction((ux, uy, vx, vy) => {
 ...
 */
const getBoundsOfCurve = codeMarkFunction(function getBoundsOfCurve(begx, begy, cp1x, cp1y, cp2x, cp2y, endx, endy) {
	let argsString;
 ...
 */
const fromArcToBeziers = codeMarkFunction((fx, fy, _ref) => {
 ...
 */
const makePathSimpler = codeMarkFunction(path => {
 ...
 */
const calcLineLength = codeMarkFunction((x1, y1, x2, y2) => Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2), 'calcLineLength');
 ...
 * @param {number} endy
 */
const getPointOnCubicBezierIterator = codeMarkFunction((begx, begy, cp1x, cp1y, cp2x, cp2y, endx, endy) => pct => {
	const c1 = CB1(pct),
		c2 = CB2(pct),
		c3 = CB3(pct),
		c4 = CB4(pct);
...
const QB2 = t => 2 * t * (1 - t);
const QB3 = t => (1 - t) ** 2;
const getTangentCubicIterator = codeMarkFunction((p1x, p1y, p2x, p2y, p3x, p3y, p4x, p4y) => pct => {
	const qb1 = QB1(pct),
		qb2 = QB2(pct),
		qb3 = QB3(pct),
...
const getPointOnQuadraticBezierIterator = codeMarkFunction((p1x, p1y, p2x, p2y, p3x, p3y) => pct => {
	const c1 = QB1(pct),
		c2 = QB2(pct),
		c3 = QB3(pct);
	return new Point(p3x * c1 + p2x * c2 + p1x * c3, p3y * c1 + p2y * c2 + p1y * c3);
}, 'getPointOnQuadraticBezierIterator');
const getTangentQuadraticIterator = codeMarkFunction((p1x, p1y, p2x, p2y, p3x, p3y) => pct => {
	const invT = 1 - pct,
...
const pathIterator = codeMarkFunction((iterator, x1, y1) => {
 ...
 */
const findPercentageForDistance = codeMarkFunction((segInfo, distance) => {
	let perc = 0,
		tmpLen = 0,
		tempP = {
			x: segInfo.x,
 ...
 */
const getPathSegmentsInfo = codeMarkFunction(path => {
	let totalLength = 0,
 ...
 * @param infos
 */
const getPointOnPath = codeMarkFunction(function (path, distance) {
 ...
 * ];
 */
const parsePath = codeMarkFunction(pathString => {
 ...
 */
const getSmoothPathFromPoints = codeMarkFunction(function (points) {
 ...
 */
const transformPath = codeMarkFunction((path, transform, pathOffset) => {
	if (pathOffset) {
 ...
 */
const getRegularPolygonPath = codeMarkFunction((numVertexes, radius) => {
 ...
 */
const joinPath = codeMarkFunction((pathData, fractionDigits) => pathData.map(segment => {
