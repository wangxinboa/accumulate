...
const convertPercentUnitsToValues = codeMarkFunction(function convertPercentUnitsToValues(valuesToConvert, _ref) {
	let {
		width,
		height,
		gradientUnits
...
const getValue = codeMarkFunction(function getValue(el, key) {
	return el.getAttribute(key);
})
const parseLinearCoords = codeMarkFunction(function parseLinearCoords(el) {
	return {
		x1: getValue(el, 'x1') || 0,
		y1: getValue(el, 'y1') || 0,
...
const parseRadialCoords = codeMarkFunction(function parseRadialCoords(el) {
	return {
...
const parseCoords = codeMarkFunction(function parseCoords(el, size) {
