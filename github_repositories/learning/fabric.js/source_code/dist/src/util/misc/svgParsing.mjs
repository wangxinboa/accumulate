 ...
 */
const getSvgAttributes = codeMarkFunction(type => {
 ...
 * @return {number}
 */
const parseUnit = codeMarkFunction(function (value) {
...
const parseAlign = codeMarkFunction(align => {
 ...
 */
const parsePreserveAspectRatioAttribute = codeMarkFunction(attribute => {
 ...
 */
const matrixToSVG = codeMarkFunction(transform => 'matrix(' + transform.map(value => toFixed(value, config.NUM_FRACTION_DIGITS)).join(' ') + ')', 'matrixToSVG');
 ...
 * @returns
 */
const colorPropToSVG = codeMarkFunction(function (prop, value) {
...
const createSVGRect = codeMarkFunction(function (color, _ref) {
	let {
		left,
		top,
		width,
