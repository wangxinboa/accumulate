 ...
 */
const createCanvasElement = codeMarkFunction(() => {
 ...
 */
const createImage = codeMarkFunction(() => getFabricDocument().createElement('img'), 'createImage');
 ...
 * @return {CanvasElement} initialized canvas element
 */
const copyCanvasElement = codeMarkFunction(canvas => {
...
const createCanvasElementFor = codeMarkFunction(canvas => {
 ...
 */
const toDataURL = codeMarkFunction((canvasEl, format, quality) => canvasEl.toDataURL("image/".concat(format), quality), 'toDataURL');
const isHTMLCanvas = codeMarkFunction(canvas => {
