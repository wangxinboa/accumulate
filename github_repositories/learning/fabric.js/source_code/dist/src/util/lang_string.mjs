 ...
 */
const capitalize = codeMarkFunction(function (string) {
 ...
 */
const escapeXml = codeMarkFunction(string => string.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/'/g, '&apos;').replace(/</g, '&lt;').replace(/>/g, '&gt;'), 'escapeXml');
 ...
 */
const graphemeSplit = codeMarkFunction(textstring => {
	const graphemes = [];
...
const getWholeChar = codeMarkFunction((str, i) => {
