 ...
 * @return {Number}
 */
const hue2rgb = codeMarkFunction((p, q, t) => {
	if (t < 0) {
		t += 1;
	}
	if (t > 1) {
 ...
 */
const rgb2Hsl = codeMarkFunction((r, g, b, a) => {
	r /= 255;
	g /= 255;
	b /= 255;
...
const fromAlphaToFloat = codeMarkFunction(function () {
 ...
 */
const hexify = codeMarkFunction(value => Math.min(Math.round(value), 255).toString(16).toUpperCase().padStart(2, '0'), 'hexify');

/**
 * Calculate the grey average value for rgb and pass through alpha
 */
const greyAverage = codeMarkFunction(_ref => {
	let [r, g, b, a = 1] = _ref;
