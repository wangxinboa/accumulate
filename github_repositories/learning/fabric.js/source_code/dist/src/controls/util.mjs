 ...
 */
const getActionFromCorner = codeMarkFunction((alreadySelected, corner, e, target) => {
 ...
 */
const isTransformCentered = codeMarkFunction(function isTransformCentered(transform) {
	return resolveOrigin(transform.originX) === resolveOrigin(CENTER) && resolveOrigin(transform.originY) === resolveOrigin(CENTER);
})
const invertOrigin = codeMarkFunction(function invertOrigin(origin) {
	return -resolveOrigin(origin) + 0.5;
})
const isLocked = (target, lockingKey) => target[lockingKey];
const commonEventInfo = codeMarkFunction((eventData, transform, x, y) => {
	return {
		e: eventData,
		transform,
 ...
 */
const findCornerQuadrant = codeMarkFunction(function findCornerQuadrant(fabricObject, control) {
 ...
 */
const normalizePoint = codeMarkFunction(function normalizePoint(target, point, originX, originY) {
 ...
 */
const getLocalPoint = codeMarkFunction(function getLocalPoint(_ref, originX, originY, x, y) {
	var _target$canvas;
	let {
		target,
		corner
