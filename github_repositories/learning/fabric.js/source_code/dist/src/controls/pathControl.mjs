...
const calcPathPointPosition = codeMarkFunction((pathObject, commandIndex, pointIndex) => {
	const {
		path,
		pathOffset
	} = pathObject;
...
const movePathPoint = codeMarkFunction((pathObject, x, y, commandIndex, pointIndex) => {
	const {
		path,
		pathOffset
	} = pathObject;
 ...
 */
const pathPositionHandler = codeMarkFunction(function pathPositionHandler(dim, finalMatrix, pathObject) {
	const {
		commandIndex,
		pointIndex
	} = this;
 ...
 */
const pathActionHandler = codeMarkFunction(function pathActionHandler(eventData, transform, x, y) {
	const {
		target
	} = transform;
	const {
...
const PathPointControl = codeMarkClass(class PathPointControl extends Control {
	constructor(options) {
		super(options);
	}
...
const PathControlPointControl = codeMarkClass(class PathControlPointControl extends PathPointControl {
	constructor(options) {
		super(options);
	}
	render(ctx, left, top, styleOverride, fabricObject) {
...
const createControl = codeMarkFunction((commandIndexPos, pointIndexPos, isControlPoint, options, connectToCommandIndex, connectToPointIndex) => new (isControlPoint ? PathControlPointControl : PathPointControl)(_objectSpread2(_objectSpread2({
...
const createPathControls = codeMarkFunction(function createPathControls(path) {
