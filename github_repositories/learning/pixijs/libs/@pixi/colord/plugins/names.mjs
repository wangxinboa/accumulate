 ...
 */
var namesPlugin = function (ColordClass, parsers) {
    ...
    var HEX_NAME_STORE = {};
    for (var name_1 in NAME_HEX_STORE)
        HEX_NAME_STORE[NAME_HEX_STORE[name_1]] = name_1;
    ...
    var getDistanceBetween = function (rgb1, rgb2) {
    ...
    ColordClass.prototype.toName = function (options) {
        ...
        var exactMatch = HEX_NAME_STORE[this.toHex()];
        if (exactMatch)
            return exactMatch;
        // Find closest color, if there is no exact match and `approximate` flag enabled
        if (options === null || options === void 0 ? void 0 : options.closest) {
            var rgba = this.toRgb();
            var minDistance = Infinity;
            var closestMatch = "black";
            // Fill the dictionary if empty
            if (!NAME_RGBA_STORE.length) {
                for (var name_2 in NAME_HEX_STORE) {
                    NAME_RGBA_STORE[name_2] = new ColordClass(NAME_HEX_STORE[name_2]).toRgb();
                }
            }
            // Find the closest color
            for (var name_3 in NAME_HEX_STORE) {
                var distance = getDistanceBetween(rgba, NAME_RGBA_STORE[name_3]);
    ...
    var parseColorName = function (input) {
        ...
        var hex = name === "transparent" ? "#0000" : NAME_HEX_STORE[name];
        if (hex)
            return new ColordClass(hex).toRgb();
        return null;
    };
