...
class RenderGroup {
	/** @type {"renderGroup"} */
	renderPipeId;
	/** @type {PixijsType.AllContainer | null} */
	root;
	/** @type {Record<string, {index: number; list: Array<PixijsType.AllContainer>}>} */
	childrenToUpdate;
	/** @type {InstructionSet} */
	instructionSet;
	constructor() {
	...
	/**
	 * @param {PixijsType.AllContainer} root
	 */
	init(root) {
		this.root = root;
	...
	/**
	 * @param {PixijsType.AllContainer} renderable
	 */
	updateRenderable(renderable) {
		if (renderable.globalDisplayStatus < 7) return;
		/** @type {PixijsType.SpritePipe} */ (this.instructionSet.renderPipes[renderable.renderPipeId]).updateRenderable(
			renderable,
		);
